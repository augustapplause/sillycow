...
def render_major_events_section(profile, key_prefix="main"):
    ...
    # Pandas Styler formatters do not receive row context reliably across Streamlit
    # versions, so pre-format display values before styling.
    setup_summary_display = setup_summary.copy()
    for idx, row in setup_summary.iterrows():
        metric_name = row["Metric"]
        for col in ["Current Setup", "Avg Positive Setup", "Avg Negative Setup"]:
            setup_summary_display.loc[idx, col] = _fmt_setup_cell(row[col], metric_name)
