...
def get_major_event_setup_stats(profile, threshold_pct=10):
    ...
    for raw_col in ["Compression_Ratio", "CLV_Trend", "RS_20", "Volume_Ratio"]:
        df[f"Setup_{raw_col}"] = df[raw_col].shift(1)

    events = df.dropna(
        subset=["Dollar_Change_Event_Window", "Percent_Change_Event_Window"] + list(event_metric_map.keys())
    ).copy()
        ...
        detail_rows.append(
            {
                ...
                "Relative Strength (20D)": row["Setup_RS_20"],
