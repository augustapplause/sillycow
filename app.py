...
def scan_similar_setups(
    ...
    mask = (
        candidates["Compression_Ratio"] - current["Compression_Ratio"]
    ).abs() <= compression_tolerance_ratio
    ...
    # candidate closest to the current compression ratio.
    out["_Compression_Distance"] = (
        out["Compression_Ratio"] - current["Compression_Ratio"]
    ).abs()
    out = out.sort_index()
...
def render_major_events_section(profile, key_prefix="main"):
    ...
    setup_summary = pd.DataFrame(
        [
            {
                "Setup Profile": "Current Setup",
                "Compression": current_vector["Compression"],
                "CLV": current_vector["CLV"],
                "Relative Strength": current_vector["Relative Strength"],
                "Volume Support": current_vector["Volume Support"],
                "Similarity": "",
            },
            {
                "Setup Profile": "Avg Positive Setup",
                "Compression": _series_value(positive_avg, "Compression"),
                "CLV": _series_value(positive_avg, "CLV"),
                "Relative Strength": _series_value(positive_avg, "Relative Strength"),
                "Volume Support": _series_value(positive_avg, "Volume Support"),
                "Similarity": "",
            },
            {
                "Setup Profile": "% of Positive Setup",
                "Compression": "",
                "CLV": "",
                "Relative Strength": "",
                "Volume Support": "",
            ...
            {
                "Setup Profile": "Avg Negative Setup",
                "Compression": _series_value(negative_avg, "Compression"),
                "CLV": _series_value(negative_avg, "CLV"),
                "Relative Strength": _series_value(negative_avg, "Relative Strength"),
                "Volume Support": _series_value(negative_avg, "Volume Support"),
                "Similarity": "",
            },
            {
                "Setup Profile": "% of Negative Setup",
                "Compression": "",
                "CLV": "",
                "Relative Strength": "",
                "Volume Support": "",
    ...
    def _profile_row_style(row):
        profile_name = str(row.get("Setup Profile", ""))
        if "Positive" in profile_name:
            return ["color: green; font-weight: 850;" for _ in row]
        if "Negative" in profile_name:
            return ["color: red; font-weight: 850;" for _ in row]
        return ["font-weight: 850;" if profile_name == "Current Setup" else "" for _ in row]
    ...
    summary_styled = setup_summary.style.format(
        {
            ...
            "Similarity": _fmt_similarity,
        }
    ).apply(_profile_row_style, axis=1)
