...
def download_ohlcv(ticker: str, period: str):
    ...
    if ticker == "SPY":
        st.error(f"V12.34 SPY RAW SHAPE: {raw.shape}")
        st.write("SPY RAW COLUMNS")
        st.write(raw.columns)
        st.write("SPY RAW HEAD")
        st.write(raw.head())
        st.write("SPY RAW TAIL")
        st.write(raw.tail())
...
def build_profile(
    ...
    return {
        ...
        # Keep the unfiltered OHLCV data so Major Events can scan the full
        # selected share-price history instead of the indicator-filtered setup data.
        "raw_df": raw,
        "df": df,
...
def inject_custom_css():
    st.markdown(
        ...
        /* Header/sidebar visibility fixes.
           Do NOT target generic button[kind="header"] here: on Streamlit Cloud
           that can be the GitHub/source button, not the sidebar control. */
        ...
        @media (hover: none) and (pointer: coarse) {
            ...
            /* Mobile sidebar handle: force visible contrast and move it away
               from Streamlit Cloud's GitHub/source badge in the top-left. */
            [data-testid="collapsedControl"],
            [data-testid="stSidebarCollapsedControl"],
            [data-testid="stSidebarCollapseButton"],
            button[aria-label="Open sidebar"],
...
def inject_mobile_sidebar_fallback_button():
    """Add a parent-page mobile fallback button for Streamlit sidebar open/close.

    Some mobile Streamlit deployments can hide the native sidebar control behind
    the GitHub/source-control badge or app chrome. CSS alone cannot show a
    control that Streamlit did not render or that another overlay is covering.
    This injects a small fixed button into the parent document and delegates the
    click to Streamlit's real sidebar open/close button when it exists.
    """
    components.html(
        """
        <script>
        (function () {
            ...
            const buttonId = "stock-profiler-mobile-sidebar-fallback";
            const styleId = "stock-profiler-mobile-sidebar-fallback-style";
            ...
            if (!btn) {
                ...
                btn.id = buttonId;
                btn.type = "button";
                btn.setAttribute("aria-label", "Open or close sidebar");
                btn.innerHTML = "☰";
            ...
            function findSidebarControl() {
                ...
                for (const selector of selectors) {
                    ...
                    const match = candidates.find(visible) || candidates[0];
            ...
            btn.onclick = function () {
                const nativeControl = findSidebarControl();
                if (nativeControl) {
                    nativeControl.click();
                    return;
                }
                const app = doc.querySelector('[data-testid="stAppViewContainer"]');
                if (app) app.dispatchEvent(new KeyboardEvent('keydown', {key: 'b', code: 'KeyB', ctrlKey: true, bubbles: true}));
            };
        })();
        </script>
        """,
...
def compute_major_events_40d(profile, threshold_pct=10, lookahead_days=40):
    """Scan the full selected share-price history for major price events.

    Event detection uses raw OHLC data, so missing setup indicators do not
    prevent price events from being found. Setup metrics are attached from the
    trading day immediately before the event start when available.
    """
    price_df = profile.get("raw_df", profile["df"]).copy()
    price_df = price_df.dropna(subset=["Close", "High", "Low"]).copy()

    setup_df = profile["df"].copy()
    rs_col_map = profile.get("rs_col_map", {}) or {}
    rs_cols = [col for col in rs_col_map.values() if col in setup_df.columns]

    if len(price_df) <= lookahead_days + 1:
        return pd.DataFrame()
    ...
    def _get_setup_row(setup_date):
        """Return the latest available setup row on or before setup_date."""
        if setup_df.empty:
            return pd.Series(dtype=float)

        available = setup_df.loc[setup_df.index <= setup_date]
        if available.empty:
            return pd.Series(dtype=float)
    ...
    for start_pos in range(1, len(price_df) - lookahead_days):
        setup_pos = start_pos - 1
        event_start = dates[start_pos]
        setup_date = dates[setup_pos]
        start_close = float(price_df.iloc[start_pos]["Close"])

        if not np.isfinite(start_close) or start_close <= 0:
            continue

        forward_window = price_df.iloc[start_pos + 1 : start_pos + lookahead_days + 1]
        ...
        setup_row = _get_setup_row(setup_date)
            ...
            positive_candidates.append(
                {
                    **base,
                    ...
                    "Event Type": "Positive",
                    "Days to Event": int(price_df.index.get_loc(max_high_date) - start_pos),
            ...
            negative_candidates.append(
                {
                    **base,
                    ...
                    "Event Type": "Negative",
                    "Days to Event": int(price_df.index.get_loc(min_low_date) - start_pos),
                    "Event Price": min_low,
...
def get_major_event_setup_stats(profile, threshold_pct=10):
    ...
    def _fmt_event_change_range(series):
        if series is None or len(series) == 0:
            return ""
        clean = pd.Series(series).dropna()
        if clean.empty:
            return ""
        return f"{signed_money0(clean.min())} to {signed_money0(clean.max())}"
    ...
    def _event_subset_stats(subset):
        ...
        event_change = subset["Dollar_Change_Event_Window"].dropna()
        return {
            "count": len(subset),
            "metrics": {name: avg.get(name, np.nan) for name in metric_col_map},
            "avg_change": event_change.mean() if not event_change.empty else np.nan,
            "range": _fmt_event_change_range(event_change),
    ...
    raw_rows.extend(
        [
            ...
            },
            {
                ...
                "Major Event Avg Positive Setup": major_pos["range"],
                "Major Event Avg Negative Setup": major_neg["range"],
            },
            {
                "Metric": "Similarity",
                "Current Setup": "",
...
def render_major_events_section(profile, key_prefix="main"):
    ...
    st.caption(
        f"{event_count} major events found by scanning the full selected raw OHLC share-price history for independent 40-trading-day upside or downside moves of +/- {threshold_pct}% or greater. "
        "Setup metrics are attached from the trading day immediately before the event start when available. Scroll inside the table to view additional events."
    )
...
def main():
    st.set_page_config(page_title="Stock Setup Profiler", layout="wide", initial_sidebar_state="collapsed")
    inject_custom_css()
    inject_mobile_sidebar_fallback_button()
    ...
    if run_button or st.session_state.get("profile_request_key") != current_request_key:
        try:
            with st.spinner("Building profiles..."):
                ...
                benchmark_debug_rows = []
                for bm, bm_df in benchmark_dfs.items():
                    benchmark_debug_rows.append({
                        "Benchmark": bm,
                        "Rows": int(len(bm_df)) if bm_df is not None else 0,
                        "First Date": bm_df.index.min().date().isoformat() if bm_df is not None and not bm_df.empty else "N/A",
                        "Last Date": bm_df.index.max().date().isoformat() if bm_df is not None and not bm_df.empty else "N/A",
                        "Last Close": float(bm_df["Close"].iloc[-1]) if bm_df is not None and not bm_df.empty and "Close" in bm_df.columns else np.nan,
                    })
                st.session_state["benchmark_debug_rows"] = benchmark_debug_rows
                profiles = []
    ...
    benchmark_debug_rows = st.session_state.get("benchmark_debug_rows", [])
    if benchmark_debug_rows:
        with st.expander("Benchmark download diagnostics"):
            st.dataframe(pd.DataFrame(benchmark_debug_rows), use_container_width=True, hide_index=True)
            st.caption("Use this to confirm whether yfinance returned data for SPY and the other benchmark tickers.")
