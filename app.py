...
def get_live_price(ticker: str):
    """Return the most current yfinance price available for the ticker.

    Prefer fast_info['lastPrice']; fall back to the latest 1-minute bar and then
    to regularMarketPrice from info. Cached briefly so reruns refresh without
    ...
    try:
        ...
        last_price = fast_info.get("lastPrice") if hasattr(fast_info, "get") else fast_info["lastPrice"]
    ...
    try:
        ...
        last_price = info.get("regularMarketPrice") if isinstance(info, dict) else None
        if last_price is not None and np.isfinite(float(last_price)):
            return float(last_price)
    except Exception:
        pass
...
def build_analog_chart(profile, hvns):
    ...
    live_price = profile.get("live_price", np.nan)
