...
def get_live_quote(ticker: str):
    """Return the most current yfinance price and previous close available.

    Prefer fast_info for both values; fall back to 1-minute data for last price
    and to info/history for previous close. Cached briefly so reruns refresh
    ...
    if pd.isna(out["last_price"]):
        try:
            intraday = yticker.history(period="1d", interval="1m")
            if intraday is not None and not intraday.empty and "Close" in intraday.columns:
                last_price = intraday["Close"].dropna().iloc[-1]
    ...
    try:
        ...
        if isinstance(info, dict):
            if pd.isna(out["last_price"]):
                ...
                if last_price is not None and np.isfinite(float(last_price)):
                    out["last_price"] = float(last_price)
            if pd.isna(out["previous_close"]):
                previous_close = info.get("regularMarketPreviousClose") or info.get("previousClose")
                if previous_close is not None and np.isfinite(float(previous_close)):
                    out["previous_close"] = float(previous_close)
    except Exception:
        pass
    ...
    if pd.isna(out["previous_close"]):
        try:
            ...
            if daily is not None and not daily.empty and "Close" in daily.columns:
                ...
                if len(closes) >= 2:
                    out["previous_close"] = float(closes.iloc[-2])
                elif len(closes) == 1:
                    out["previous_close"] = float(closes.iloc[-1])
        except Exception:
            pass
...
def add_profile_columns(df: pd.DataFrame, benchmark_df: Optional[pd.DataFrame] = None) -> pd.DataFrame:
    ...
    if benchmark_df is not None and not benchmark_df.empty:
        ...
        # Some tickers/benchmarks can have slightly different latest available
        # dates from yfinance. Forward-fill the benchmark so the current setup
        # does not show a blank Relative Strength value when the benchmark close
        # is missing for the stock's latest row.
...
def compute_major_events_40d(profile, threshold_pct=10, lookahead_days=40):
    """Identify major events using a 40-trading-day look-forward window.

    Each event start date gets one dominant outcome: whichever is larger in
    absolute value between the maximum upside excursion and maximum downside
    excursion over the next 40 trading days. Setup metrics are taken from the
    trading day immediately before the event start. Overlapping candidate event
    windows are de-duplicated by keeping the strongest absolute move first.
    """
    df = profile["df"].copy()
    required_cols = ["Compression_Ratio", "CLV_Trend", "RS_20", "Volume_Ratio", "Close", "High", "Low"]
    ...
    for start_pos in range(1, len(df) - lookahead_days):
        ...
        if abs(max_gain_pct) >= abs(max_loss_pct):
            event_type = "Positive"
            event_end = max_high_date
            end_price = max_high
            event_pct = max_gain_pct
        else:
            event_type = "Negative"
            event_end = min_low_date
            end_price = min_low
            event_pct = max_loss_pct

        if abs(event_pct) < threshold_pct:
            continue

        setup_row = df.iloc[setup_pos]
        start_end_days = int(df.index.get_loc(event_end) - start_pos)
        candidates.append(
            {
                "Setup Date": setup_date.date(),
                "Event Start": event_start.date(),
                "Event End": event_end.date(),
                "Event Type": event_type,
                "Days to Event": start_end_days,
                "Start Price": start_close,
                "Event Price": end_price,
                "Dollar_Change_Event_Window": end_price - start_close,
                "Percent_Change_Event_Window": event_pct,
    ...
    # Collapse overlapping 40-day look-forward windows by keeping the strongest
    # absolute move first. This keeps the detail table from counting the same
    # broad move many times from adjacent setup dates.
    selected = []
    ...
    events = events.sort_values("Event Start", ascending=False).reset_index(drop=True)
    return events
...
def get_major_event_setup_stats(profile, threshold_pct=10):
    ...
    event_metric_map = {
        ...
        "Setup_CLV_Trend": "CLV",
        "Setup_RS_20": "Relative Strength (20D)",
        "Setup_Volume_Ratio": "Volume Support",
    }

    df = profile["df"].copy()
    latest_pool = df.dropna(subset=["Compression_Ratio", "CLV_Trend", "Volume_Ratio", "RS_20"])
    ...
    current_vector = pd.Series(
        {
            ...
            "Relative Strength (20D)": latest.get("RS_20", np.nan),
    ...
    def _event_subset_stats(subset):
        ...
        avg = subset[list(event_metric_map.keys())].rename(columns=event_metric_map).mean()
        return {
            "count": len(subset),
            "compression": avg.get("Compression", np.nan),
            "rs": avg.get("Relative Strength (20D)", np.nan),
            "clv": avg.get("CLV", np.nan),
            "volume": avg.get("Volume Support", np.nan),
    ...
    return {
        "events": events,
        "event_metric_map": event_metric_map,
...
def render_summary_metrics(profile, key_prefix="main"):
    ...
    latest_pool = profile["df"].dropna(
        subset=["Compression_Ratio", "CLV_Trend", "Volume_Ratio", "RS_20"]
    )
    latest = latest_pool.iloc[-1] if not latest_pool.empty else profile["df"].iloc[-1]
    ...
    with summary_slider_col:
        threshold_pct = st.slider(
            ...
            help=(
                "Major events use a 40-trading-day look-forward window. The app measures maximum upside and downside excursions, classifies by the dominant absolute move, keeps moves above this threshold, collapses overlapping windows by keeping the strongest event, and uses setup metrics from the trading day immediately before the event start."
            ),
        )
    ...
    def _subset_stats(subset):
        ...
        avg_vector = pd.Series(
            {
                ...
                "Relative Strength (20D)": subset["RS_20"].mean(),
                "Volume Support": subset["Volume_Ratio"].mean(),
            }
        )
        reference_values = subset[["Compression_Ratio", "CLV_Trend", "RS_20", "Volume_Ratio"]].rename(
            columns={
        ...
        return {
            "count": len(subset),
            "compression": avg_vector["Compression"],
            "rs": avg_vector["Relative Strength (20D)"],
            "clv": avg_vector["CLV"],
            "volume": avg_vector["Volume Support"],
    ...
    raw_rows = [
        ...
        },
        {
            ...
            "Major Event Avg Positive Setup": major_pos["compression"],
            "Major Event Avg Negative Setup": major_neg["compression"],
        },
        {
            "Metric": "Relative Strength (20D)",
            "Current Setup": current_vector["Relative Strength (20D)"],
            "Avg Positive Setup": positive_stats["rs"],
            "Avg Negative Setup": negative_stats["rs"],
            "Overall Avg Setup": overall_stats["rs"],
            "Major Event Avg Positive Setup": major_pos["rs"],
            "Major Event Avg Negative Setup": major_neg["rs"],
        },
        {
            "Metric": "CLV",
            ...
            "Major Event Avg Positive Setup": major_pos["clv"],
            "Major Event Avg Negative Setup": major_neg["clv"],
        },
        {
            ...
            "Major Event Avg Positive Setup": major_pos["volume"],
            "Major Event Avg Negative Setup": major_neg["volume"],
        },
        {
            "Metric": "Avg $ Change",
            "Current Setup": "",
    ...
    def _format_cell(metric, value):
        ...
        if metric == "Relative Strength (20D)":
    ...
    header_html = "".join(
        f'<th>{header_labels[col]}</th>' for col in columns
    )
    body_rows = []
    for row in raw_rows:
        metric = row["Metric"]
...
def render_major_events_section(profile, key_prefix="main"):
    ...
    detailed_styled = display_table.style.format(
        {
            ...
            "Relative Strength (20D)": lambda v: _fmt_number(v, 1),
    ...
    st.markdown(
        """
        <div class="section-title" title="Major events use a 40-trading-day look-forward window. For each event start date, the app measures the maximum upside excursion and maximum downside excursion over the next 40 trading days, classifies the event by the dominant absolute move, keeps only moves above the slider threshold, collapses overlapping 40-day windows by keeping the strongest event, and uses setup metrics from the trading day immediately before the event start.">
    ...
    st.caption(
        f"{event_count} major events where the dominant 40-trading-day look-forward move was +/- {threshold_pct}% or greater. "
