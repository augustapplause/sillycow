...
def info_tooltip(label: str, tooltip: str) -> str:
    """Return an inline HTML tooltip that works on desktop hover and mobile tap."""
    safe_label = html.escape(str(label))
    safe_tooltip = html.escape(str(tooltip)).replace("\n", "<br>")
    return f'<span class="info-tooltip" tabindex="0">{safe_label}<span class="info-tooltip-bubble">{safe_tooltip}</span></span>'
...
def render_hmm_state_section(profile, key_prefix="main"):
    ...
    if not age_conditioned.empty:
        ...
        st.markdown(f'<div class="section-title">{info_tooltip(f"Age-conditioned transition probabilities for current {latest_state} regime ({age_conditioned_label})", AGE_CONDITIONED_TOOLTIP)}</div>', unsafe_allow_html=True)
    ...
    if not state_summary.empty:
        st.markdown(
            f'<div class="section-title">{info_tooltip("Regime Characteristics", REGIME_CHARACTERISTICS_TOOLTIP)}</div>',
