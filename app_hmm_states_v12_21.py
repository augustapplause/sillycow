...
def _normal_pdf_1d(x, mean, var):
    var = max(float(var), 1e-8)
    return np.exp(-0.5 * ((x - mean) ** 2) / var) / np.sqrt(2 * np.pi * var)


def _forward_backward_1d(x, pi, trans, means, vars_):
    n = len(x)
    k = len(pi)
    emissions = np.zeros((n, k))
    for j in range(k):
        emissions[:, j] = _normal_pdf_1d(x, means[j], vars_[j]) + 1e-300
...
def fit_gaussian_hmm_1d(x, n_states=3, max_iter=80, tol=1e-5):
    x = np.asarray(x, dtype=float)
    x = x[np.isfinite(x)]
    if len(x) < 80:
        raise ValueError("At least 80 daily return observations are needed for HMM state analysis.")

    qs = np.linspace(0.2, 0.8, n_states)
    ...
    for _ in range(max_iter):
        gamma, xi_sum = _forward_backward_1d(x, pi, trans, means, vars_)
        ...
        emissions = np.column_stack([_normal_pdf_1d(x, means[j], vars_[j]) + 1e-300 for j in range(n_states)])
        alpha = pi * emissions[0]
        scale = alpha.sum() + 1e-300
        ll = np.log(scale)
        alpha /= scale
        for t in range(1, len(x)):
            alpha = emissions[t] * (alpha @ trans)
            scale = alpha.sum() + 1e-300
            ll += np.log(scale)
            alpha /= scale
    ...
    gamma, _ = _forward_backward_1d(x, pi, trans, means, vars_)
    return pi, trans, means, vars_, gamma
...
def compute_hmm_state_analysis(df: pd.DataFrame) -> dict:
    ...
    pi, trans, means, vars_, gamma = fit_gaussian_hmm_1d(returns.values, n_states=3)

    # Map fitted states to market labels by expected daily return.
    ...
    ordered_trans = trans[np.ix_(ordered_model_states, ordered_model_states)]

    state_ids = gamma.argmax(axis=1)
    state_labels = pd.Series([label_for_model_state[int(i)] for i in state_ids], index=returns.index, name="State")
    state_history = pd.DataFrame({"Daily Return %": returns, "State": state_labels})
    ...
    same_state_runs = np.array(run_lengths.get(latest_state, []), dtype=float)
    if same_state_runs.size:
        current_state_age_percentile = float((same_state_runs <= current_state_age).mean() * 100)
    else:
        current_state_age_percentile = np.nan
            ...
            rows.append(
                {
                    ...
                    "To State": to_state,
                    "Daily Transition Probability": ordered_trans[i, j] * 100,
                }
            )
    ...
    state_summary = pd.DataFrame(
        {
            "State": STATE_ORDER,
            "Avg Daily Return %": [means[ordered_model_states[i]] for i in range(3)],
            "Volatility %": [np.sqrt(vars_[ordered_model_states[i]]) for i in range(3)],
            "Days Classified": [(state_labels == state).sum() for state in STATE_ORDER],
        }
    )
...
def render_hmm_state_section(profile, key_prefix="main"):
    st.markdown('<div class="section-title">Hidden Markov Model State Probabilities ⓘ</div>', unsafe_allow_html=True)
    ...
    if transition_table.empty:
        st.info("Not enough historical data for HMM state probabilities.")
        return
    ...
    st.caption("Rows are today's inferred state and columns are the next trading day's inferred state. States are fitted from daily close-to-close returns using a 3-state Gaussian hidden Markov model.")

    state_summary = analysis.get("state_summary", pd.DataFrame())
    if not state_summary.empty:
        summary_styled = state_summary.style.format(
            {"Avg Daily Return %": "{:+.3f}%", "Volatility %": "{:.3f}%", "Days Classified": "{:,.0f}"}
        )
        st.dataframe(summary_styled, use_container_width=True, hide_index=True, height=150)
