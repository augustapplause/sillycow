...
def render_major_events_section(profile, key_prefix="main"):
    ...
    with left:
        ...
        st.info(
            "Major events are trading days where the absolute same-day close-to-close "
            "percentage move is at least the selected threshold."
        )

    df = profile["df"].copy()
    df["Dollar_Change_1D"] = df["Close"] - df["Close"].shift(1)
    df["Percent_Change_1D"] = (df["Close"] / df["Close"].shift(1) - 1) * 100

    metric_cols = ["Compression_Percentile", "CLV_Trend", "RS_20", "Volume_Ratio"]
    latest = df.iloc[-1]
    rows = [
        {
            "Event Date": f"Current State ({df.index[-1].date()})",
            "$ Change": "",
            "% Change": "",
            "Compression": latest.get("Compression_Percentile", np.nan),
            "CLV": latest.get("CLV_Trend", np.nan),
            "Relative Strength": latest.get("RS_20", np.nan),
            "Volume Support": latest.get("Volume_Ratio", np.nan),
            "_sort_date": df.index[-1],
            "_is_current": True,
        }
    ]

    events = df.dropna(subset=["Dollar_Change_1D", "Percent_Change_1D"] + metric_cols).copy()
    events = events[events["Percent_Change_1D"].abs() >= threshold_pct]
        ...
        rows.append(
            {
                ...
                "$ Change": row["Dollar_Change_1D"],
                "% Change": row["Percent_Change_1D"],
                "Compression": row["Compression_Percentile"],
                "CLV": row["CLV_Trend"],
                "Relative Strength": row["RS_20"],
                "Volume Support": row["Volume_Ratio"],
    ...
    display_table = table.drop(columns=["_sort_date", "_is_current"])
    ...
    styled = display_table.style.format(
        {
            "$ Change": _fmt_money,
            "% Change": _fmt_pct,
            "Compression": lambda v: _fmt_number(v, 1),
    ...
    visible_rows = min(len(display_table), 10)
    height = 38 * (visible_rows + 1)
    with right:
        st.dataframe(styled, use_container_width=True, hide_index=True, height=height)
        event_count = max(len(display_table) - 1, 0)
        st.caption(
            f"Current state plus {event_count} major events at +/- {threshold_pct}% or greater. "
            "Scroll inside the table to view additional events."
        )
