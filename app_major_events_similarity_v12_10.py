...
def render_summary_metrics(profile, key_prefix="main"):
    ...
    metric_keys = ["Compression", "CLV", "Relative Strength (20D)", "Volume Support"]
    current_vector = pd.Series(
        {
            "Compression": latest.get("Compression_Ratio", np.nan),
            "CLV": latest.get("CLV_Trend", np.nan),
    ...
    for idx, row in display.iterrows():
        ...
        for col in columns[1:]:
            ...
            elif metric == "Relative Strength (20D)":
    ...
    st.dataframe(styled, use_container_width=True, hide_index=True, height=382)
...
def render_major_events_section(profile, key_prefix="main"):
    ...
    setup_summary = pd.DataFrame(
        [
            ...
            },
            {
                "Metric": "Relative Strength (20D)",
                "Current Setup": current_vector["Relative Strength (20D)"],
                "Avg Positive Setup": _metric_avg(positive_avg, "Relative Strength (20D)"),
                "Avg Negative Setup": _metric_avg(negative_avg, "Relative Strength (20D)"),
                "Closest": _closest_profile("Relative Strength (20D)"),
            },
            {
                "Metric": "Volume Support",
                "Current Setup": current_vector["Volume Support"],
    ...
    if "Closest" in setup_summary.columns:
        setup_summary = setup_summary.drop(columns=["Closest"])
    ...
    def _fmt_setup_cell(value, metric_name):
        ...
        if metric_name == "Relative Strength (20D)":
    ...
    with right:
        ...
        st.caption(
            "Similarity uses a standardized vector distance across Compression, CLV, Relative Strength (20D), and Volume Support. "
